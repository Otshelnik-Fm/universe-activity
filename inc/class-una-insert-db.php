...
class UNA_Insert_DB {
    ...
    public function insert_db( $argum ) {
        ...
        // исключаем дубликаты
        if ( $this->una_check_duplicate( $args ) )
            return false;
    ...
    private function una_check_duplicate( $args ) {
        ...
        $duplicate = $wpdb->get_row(
			...
			AND `hide` = '%d'
			AND `group_id` = '%d'
                ;", $args['user_id'], $args['action'], $args['act_date'], $args['object_id'], $args['object_name'], $args['object_type'], $args['subject_id'], $args['other_info'], $args['user_ip'], $args['hide'], $args['group_id']
            )
        );
