...
class UNA_Register_Type_Callback {
    ...
    public function get_type_callback() {
        $type  = array(
            ...
            'group_change_desc'      => array( // установил в группе описание, сменил его
                'callback' => 'una_get_group_change_desc',
            ),
            'group_user_ban'         => array( // получил бан в группе
                'callback' => 'una_get_group_user_change_role',
                'access'   => 'logged',
            ),
            'group_user_role'        => array( // в группе сменилась роль юзера
                'callback' => 'una_get_group_user_change_role',
                'access'   => 'logged',
            ),
            'group_is_closed'        => array( // группа: открытая/закрытая
